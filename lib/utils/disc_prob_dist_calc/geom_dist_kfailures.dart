/// Computes geometric distribution (k failures) and almost all of its related cases (probabilities and statistics).
/// 
/// Abbreviations:
/// 1. E = Equals
/// 2. LT = Less than
...
class GeometricDistributionKFailures {
  ...
  static double _geomDistE(p, x) {
    return _geomFormulaKFailures(p, x);
  }
  ...
  static double _geomDistLTE(p, x) {
    double result = 0.0;
    for (int i = 0; i <= x; i++) result += _geomFormulaKFailures(p, i);
    return result;
  }
  ...
  /// Utility function that uses GDKF formula.
  static double _geomFormulaKFailures(p, x) {
    return p * pow(1 - p, x);
  }
}
